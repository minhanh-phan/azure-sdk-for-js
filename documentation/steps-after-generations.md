...
After this finishes, you will see the generated code in `src` folder in your **{PROJECT_ROOT}**. Refer to [the development workflows guide](../CONTRIBUTING.md#development-workflows) for more details if you'd like to know more.
...
After you build your package, install and run the release tool from the repo root:
```
...
In order to release it, we need to add some tests for it to make sure we are delivering high quality packages. After generation you will see a starter test file in your `{PROJECT_ROOT}/test/public` folder, which has an empty test and you could add/update test cases against your own services.
...
After [writing and running test cases](#how-to-write-test-for-dpgrlc), you need to push the recordings to [assets repo](https://github.com/Azure/azure-sdk-assets). Please refer to the [push recording guide](./Quickstart-on-how-to-write-tests.md#how-to-push-test-recordings-to-assets-repo) to push recordings.
