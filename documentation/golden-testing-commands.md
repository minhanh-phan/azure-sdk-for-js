...
| `pnpm turbo build --filter=@azure/<package-name>... --token 1` | Builds one package and all workspace dependencies from the repo root. |
|                         |                                                                                                                 |
...
```bash
pnpm run integration-test -- --testNamePattern "simple"
```
